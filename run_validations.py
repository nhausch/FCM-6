"""
Phase 1–3 validations: core correctness, conditioning, and error/stability.
Run with: python run_validations.py
Or from main: python main.py (no --task) calls run_all() here.
"""
...
def run_phase2_validation(a=1.0, b=3.0, n=5, grid_size=200, dtype=np.float64):
    """Phase 2: kappa_xy, kappa_x1 on a grid; Lambda_n, Hn; sanity checks."""
    x_grid = np.linspace(a, b, grid_size, dtype=dtype)
...
def run_phase3_validation(a=1.0, b=3.0, n=8, grid_size=200):
    """Phase 3: precision, forward error, stability ratio, compare_to_bound."""
...
def run_all():
    """Run Phase 1, 2, and 3 validations and print results."""
    _run_phase1()
    print("\n--- Phase 2 validation ---")
    for dtype in (np.float64, np.float32):
        name = "float64" if dtype == np.float64 else "float32"
