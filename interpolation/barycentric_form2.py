...
# Same weights as Form 1 (gamma) so the interpolant matches.
def compute_beta(x, dtype=np.float64):
    return compute_gamma(x, dtype)

# Computes beta and y = f(x) for Barycentric Form 2.
def setup_barycentric2(x, f, dtype=np.float64):
    x = np.asarray(x, dtype=dtype).ravel()
    beta = compute_beta(x, dtype)
